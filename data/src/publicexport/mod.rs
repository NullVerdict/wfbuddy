		...
		Ok(Self {
			relic_arcane_url: manifest_url(
				select_url(&urls, "ExportRelicArcane")
					.ok_or_else(|| anyhow::anyhow!("index didn't contain ExportRelicArcane"))?,
			),
			recipes_url: manifest_url(
				select_url(&urls, "ExportRecipes")
					.ok_or_else(|| anyhow::anyhow!("index didn't contain ExportRecipes"))?,
			),
			resources_url: manifest_url(
				select_url(&urls, "ExportResources")
					.ok_or_else(|| anyhow::anyhow!("index didn't contain ExportResources"))?,
			),
			warframes_url: manifest_url(
				select_url(&urls, "ExportWarframes")
					.ok_or_else(|| anyhow::anyhow!("index didn't contain ExportWarframes"))?,
			),
			weapons_url: manifest_url(
				select_url(&urls, "ExportWeapons")
					.ok_or_else(|| anyhow::anyhow!("index didn't contain ExportWeapons"))?,
			),
			sentinels_url: manifest_url(
				select_url(&urls, "ExportSentinels")
					.ok_or_else(|| anyhow::anyhow!("index didn't contain ExportSentinels"))?,
			),
		})
	}
